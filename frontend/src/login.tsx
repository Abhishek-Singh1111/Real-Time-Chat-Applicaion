  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("http://localhost:8000/api/auth/login", {
        ...
        body: JSON.stringify(formData), // Sends { email, password }
      });
      ...
      if (response.ok) {
        ...
        if (data.message === "User not found" || data.message === "Invalid email or password") {
          setMessage({ text: "User does not exist. Please sign up first.", type: "error" });
        } else {
